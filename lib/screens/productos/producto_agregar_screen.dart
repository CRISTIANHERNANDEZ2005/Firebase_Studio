...
class _ProductoAgregarScreenState extends State<ProductoAgregarScreen> {
  ...
  final _nombreController = TextEditingController(); // Controla el campo de nombre
  final _precioController = TextEditingController(); // Controla el campo de precio
  final _descripcionController = TextEditingController(); // Controla descripción
  final _nombreCategoriaController = TextEditingController(); // Controla categoría
  ...
  String? _validatePrecio(String? value) {
    if (value == null || value.isEmpty) return 'Precio es requerido'; // Validación de campo requerido
    final price = double.tryParse(value); // Intenta convertir a double
    if (price == null || price <= 0) return 'Ingrese un precio válido'; // Debe ser número positivo
    return null; // Retorna null si es válido
  }

  // Función para agregar un nuevo producto
  void _agregarProducto() async {
    if (_formKey.currentState!.validate()) { // Primero valida el formulario
      ...
      final error = await productoService.createProducto(
        ...
        precio: double.parse(_precioController.text), // Precio convertido a double
        descripcion: _descripcionController.text.isEmpty 
            ? null // Descripción opcional
            : _descripcionController.text,
        nombreCategoria: _nombreCategoriaController.text.isEmpty
            ? null // Categoría opcional
            : _nombreCategoriaController.text,
      );

      // Manejo de resultados
      if (error == null && mounted) { // Si no hay errores
        Navigator.pop(context, 'Producto creado correctamente'); // Cierra con mensaje de éxito
      } else if (mounted) { // Si hay error
  ...
  Widget build(BuildContext context) {
    final productoService = Provider.of<ProductoService>(context); // Accede al servicio
    ...
    return EditFormContainer(
      ...
      formFields: [ // Lista de campos del formulario
        Form(
